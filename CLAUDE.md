...
**The current requirements are this file and [`docs/DECISIONS.md`](docs/DECISIONS.md).** Read the
decisions before changing anything structural. [`docs/APP_BRIEF.md`](docs/APP_BRIEF.md) is the
original 2026-08-27 brief, frozen as the record of what was decided and why — read it for history,
never build from it.

## What this is

A Dutch question deck for two people, played on a phone. One editable file, one generated one:
...
1. **Never run git in this folder.** Not `commit`, not `add`, not bare `status`. The owner commits in
...
6. **The private layer never crosses into this repository.** Tijdcapsule cards, the players'
   names, the thumbs and the memory export file are *facts about two people*. They live on the
   phone and in an exported file, and there is **no code path — and no manual one — that moves
   them into `cards.json`, into any committed file, or into a session transcript that could reach
   one.** A Tijdcapsule card is not promotable to the deck: the app must refuse, and so must you.
...
- **No memory export.** `vragenspel-geheugen-*.json` and anything derived from one. It is the
  private layer in a file — Tijdcapsule cards, names, thumbs, own cards. Gitignored by pattern,
  and never quoted into a doc, a changelog or a commit message either. **If you are handed one,
  read what you need and quote nothing from it.** (`docs/DECISIONS.md` D20)
...
- **Clarity over cleverness.** The owner is not a software engineer and will not debug this. Code
  arrives working, and jargon is explained on first use.
...
- **`file://` blocks `fetch()` — settled, do not reopen.** The app loads the deck from the
  ...
  and run the build. (`docs/DECISIONS.md` D12)
...
`python build_vragenspel.py --check` regenerates in memory and reports whether `cards.js` is
already current, without writing. Use it to prove a change did what you expected.

**The printable deck was retired on 2026-08-28** (`docs/DECISIONS.md` D17). The build no longer
emits `VRAGENSPEL.html` and the layout templates are gone. Do not bring it back without reading
D17 first — the paper deck and the app had drifted into two different games.

## Before you hand anything over

- **When a change reverses something [`docs/APP_BRIEF.md`](docs/APP_BRIEF.md) argues for,
  annotate that passage in the same change.** Mark it at the line — `[SUPERSEDED — D<n>]` — and
  never rewrite it into agreement: a brief edited to match what shipped stops being evidence of
  what was decided. A banner at the top is not enough on its own, because a reader absorbs a
...
- **Anything the owner runs gets a double-clickable launcher**, not a command to paste, and it pauses
